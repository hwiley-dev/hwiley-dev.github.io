# Project State

## Current Goal
Expand the static portfolio site for `hunter-wiley.com` with an AI-assisted research library for fellowship review while keeping contact information, navigation, and GitHub Pages deployment assets accurate.

## Active Tasks
[ ] Review site copy and project pages for completeness and freshness.
[ ] Confirm the GitHub Pages deployment state still matches the repository configuration, including the committed `CNAME`.
[ ] Review extracted research paper text for hand-polished formatting, especially tables, citations, and diagrams.
[ ] Decide how to integrate future Substack links and a separate original philosophy/pre-generative-AI writing section.

## Completed Tasks
[x] Built a static HTML and CSS portfolio site for `hunter-wiley.com`.
[x] Added core pages for home, about, contact, and project detail content.
[x] Committed the `CNAME` file needed for the custom domain configuration.
[x] Updated public contact email to `wileyhntr@gmail.com`.
[x] Added `/research/` as an AI-assisted research library with readable on-page text and PDF links.
[x] Copied six research PDFs into deployable site assets.
[x] Added Research navigation across the home page, subpages, and project pages.
[x] Added a home page research callout that links to the new research library.
[x] Cleaned Research links to use directory URLs instead of exposing `index.html`.
[x] Audited the research library update: verified local links, clean Research URLs, email replacement, six PDF assets, and six readable paper sections.

## Known Issues
- No automated test or build pipeline is present; site verification is currently manual.
- Project state tracking was missing until this file was initialized.
- Research page text is automatically extracted from PDFs; dense tables, diagrams, and citation blocks may need manual cleanup for publication quality.

## Next Decision Required
Decide whether to prioritize Substack link integration, hand-polishing the extracted research text, or adding the separate original philosophy writing section.
